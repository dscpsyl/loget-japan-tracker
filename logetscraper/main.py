...
def main():
    ...
    logger.info(f"There are a total of {len(existingCardIds)} cards in the database already.")

    response = r.get(CARDLISTURL)
    logger.info(f"Response status code of the card list URL page: {response.status_code}")
    soup = BeautifulSoup(response.content, 'html.parser')

    cardIds, cardImgs = findCards(soup)
    assert len(cardIds) == len(cardImgs), f"Number of card Ids and card images do not match: {len(cardIds)} vs {len(cardImgs)}"
    ...
    for cardId, cardImg in zip(cardIds, cardImgs):
        ...
        cur.execute('INSERT INTO tracker_logetcards ("Id", "Name", "Img", "SpotmapLink", "LoGetURL", "SpotWebsiteLink") VALUES (%s, %s, %s, %s, %s, %s);', (cardId, name, imgURL, mapURL, cardURL, spotLink))
        conn.commit()
