    ...
    for article in soup.find_all('article'):
        for a in article.find_all('a'):
            assert a['href'].startswith(
                'list.aspx?card='), f"Unexpected URL format in finding card Ids: {a['href']}"
            u = urlparse(a['href'])[4].split('=')
            cardIds.append(u[1])

        for img in article.find_all('img'):
            assert img['src'].startswith(
                './img/cards/'), f"Unexpected URL format in finding card Ids: {img['src']}"
            v = img['src'].split('/')
            cardImg.append(v[-1])
...
def findCardName(cardURL):
    ...
    assert len(
        s) == 1, f"Unexpected number of sections in finding card name: {len(s)}"

    head = s[0].find_all('h1')

    assert len(
        head) == 1, f"Unexpected number of h1 tags in finding card name: {len(head)}"
...
def findCardSpotLink(cardURL):
    ...
    assert len(
        spot) == 1, f"Unexpected number of a tags in finding card name: {len(spot)}"

    link = spot[0].find_all('a', target='_blank')

    assert len(
        link) == 1, f"Unexpected number of a links in finding card name: {len(link)}"

    return link[0]['href']


def main():
    log.basicConfig(level=log.INFO, filename=f'scrapper_{datetime.now()}.log',
    ...
    assert len(cardIds) == len(
        cardImgs), f"Number of card Ids and card images do not match: {len(cardIds)} vs {len(cardImgs)}"
    logger.info(
    ...
    for cardId, cardImg in zip(cardIds, cardImgs):
        ...
        cur.execute('INSERT INTO tracker_logetcards ("Id", "Name", "Img", "SpotmapLink", "LoGetURL", "SpotWebsiteLink") VALUES (%s, %s, %s, %s, %s, %s);',
