...
class LoGetCards(models.Model):
    ...
    Img = models.URLField(validators=[URLValidator(), URLStartCheckValidator(
        "https://loget-card.jp/img/cards")])
    SpotmapLink = models.URLField(validators=[URLValidator(
    ), URLStartCheckValidator("https://loget-card.jp/list_map.aspx")])
    LoGetURL = models.URLField(validators=[URLValidator(
